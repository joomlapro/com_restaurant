...
class RestaurantViewDishes extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$doc->link = JRoute::_('index.php?option=com_restaurant&view=dishes');
		...
		foreach ($rows as $row)
		{
			...
			// Url link to article.
			$link = JRoute::_(RestaurantHelperRoute::getArticleRoute($row->slug, $row->catid));

			// Get row description.
			$db = JFactory::getDbo();
			$query = 'SELECT' . $db->quoteName('description') . 'FROM #__restaurant_dishes WHERE id =' . $row->id;
			$db->setQuery($query);
			...
			// Add readmore link to description if introtext is shown, show_readmore is true and description exists.
			if (!$params->get('feed_summary', 0) && $params->get('feed_show_readmore', 0) && $row->description)
			{
				$description .= '<p class="feed-readmore"><a target="_blank" href ="' . $item->link . '">' . JText::_('COM_RESTAURANT_FEED_READMORE') . '</a></p>';
			}
