...
/**
 * Restaurant Component Query Helper.
 *
 * @static
 * @package     Restaurant
 * @subpackage  com_restaurant
...
class RestaurantHelperQuery
{
	 ...
	 *
	 * @param   JRegistry  $params  An options object for the dish.
	 *
	...
	public static function buildVotingQuery($params = null)
	{
		...
		if ($voting)
		{
			...
			$join   = ' LEFT JOIN #__restaurant_rating AS v ON a.id = v.dish_id';
		}
		else
		{
			$select = '';
	...
	/**
	 * Method to order the intro dishes array for ordering
	 * down the columns instead of across.
	 * The layout always lays the introtext dishes out across columns.
	 * Array is reordered so that, when dishes are displayed in index order
	 * across columns in the layout, the result is that the
	 * desired dish ordering is achieved down the columns.
	 *
	 * @param   array    &$dishes   Array of intro text dishes.
	 ...
	 * @since   3.2
	 */
	public static function orderDownColumns(& $dishes, $numColumns = 1)
	{
		$count = count($dishes);

		// Just return the same array if there is nothing to change.
		if ($numColumns == 1 || !is_array($dishes) || $count <= $numColumns)
		{
			$return = $dishes;
		}
		// We need to re-order the intro dishes array.
		else
		{
			// We need to preserve the original array keys.
			$keys     = array_keys($dishes);
			...
			// Layout the dishes in column order, skipping empty cells.
			$i = 0;
			...
			// Now read the $index back row by row to get dishes in right row/col
			...
			for ($row = 1; ($row <= $maxRows) && ($i < $count); $row++)
			{
				for ($col = 1; ($col <= $numColumns) && ($i < $count); $col++)
				{
					$return[$keys[$i]] = $dishes[$index[$row][$col]];
